  ...
  <div class="col-lg-6">
    <div class="card">
      <div class="card-header">
        <strong>Список адресов для уведомлений</strong>
        <small> </small>
      </div>
      <div class="card-body card-block">
        <div class="row form-group">
          ...
          <div class="col-12 col-md-9">
            <p class="form-control-static">Email адреса</p>
          </div>
        </div>
        <div class="row form-group">
          <div class="col col-md-3">
            <label for="textarea-input" class=" form-control-label">Список Email адресов</label>
          </div>
          <div class="col-12 col-md-9">
            <textarea v-model="model.email_list"  rows="9" placeholder="username@example.org"  class="form-control"></textarea>
            <div class="alert alert-danger" v-if="form.errors['email_list']">
              {{ form.errors['email_list'][0] }}
            </div>
          </div>
        </div>
        <div class="row form-group">
...
export default {
  ...
  methods: {
    ...
    async get() {
      $axios
        .get('/api/email-notification/get')
        .then((response) => {
          const modelData = response.data.data;
          modelData.email_list = modelData.email_list.join('\n');
